...
- `skyforge.forwardCluster.upstreamWorkerRuntime.*`: Post-Helm runtime patches for upstream-owned Forward compute/search workers. Use these to enforce explicit worker heap caps when the upstream Forward chart does not expose a first-class memory-limit knob for those pods.
